...
override LDFLAGS = -X content-verify-log/pkg/util.version=$(VERSION) \
-X content-verify-log/pkg/util.buildDate=${BUILD_DATE} \
-X content-verify-log/pkg/util.gitCommit=${GIT_COMMIT} \
-X content-verify-log/pkg/util.gitTreeState=${GIT_TREE_STATE}

ifneq ($(GIT_TAG),)
override LDFLAGS += -X content-verify-log/pkg/util.gitTag=${GIT_TAG}
endif
